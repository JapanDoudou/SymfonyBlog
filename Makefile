...
clear-cache: ## Clear all caches
	...
	$(DOCKER_COMPOSE_DEV) run --rm php bash -ci 'php bin/console cache:clear'
...
start: ## Start all the stack (you can access the project on localhost:8080 after that)
	@-docker network create my-database_stack
	$(DOCKER_COMPOSE_DEV) up -d
	$(DOCKER_COMPOSE_DEV) run --rm php bash -ci './clear-cache.sh'
	@echo "Now you can access http://localhost:8088"

stop: ## Stop all the containers that belongs to the project
	@-docker network disconnect my-database_stack $$(docker ps --filter "name=my-database_php_*" -q)
...
db-drop: ## Drop Database ! Do not use in prod
	$(DOCKER_COMPOSE_DEV) run --rm php bash -ci "php bin/console d:d:drop --force"

create-test-env: ## Clear then Init test database
	$(DOCKER_COMPOSE_DEV) run --rm php bash -ci 'php bin/console doctrine:database:drop --force --env=test'
	$(DOCKER_COMPOSE_DEV) run --rm php bash -ci 'php bin/console doctrine:database:create --env=test'
	$(DOCKER_COMPOSE_DEV) run --rm php bash -ci 'php bin/console doctrine:migrations:migrate -n --env=test'

phpunit: ## launch PHPUNit tests
	$(DOCKER_COMPOSE_DEV) run --rm php "bin/phpunit"

grumphp-install: ## Install grumphp hook
	$(DOCKER_COMPOSE_DEV) run --rm php bash -ci 'php ./vendor/bin/grumphp git:init'
